...
- Добавлена вкладка `VR -> Mouse`: Delete показывает/скрывает overlay, есть опция постоянного зажатия ПКМ и ползунок DPI для чувствительности mouse-look.

- Оверлей открывается на Delete как отдельное окно, при этом фон затемняется; добавлен более декорированный UI приложения.

- Для выбранного `OpenXR Runtime` добавлен автозапуск перед стартом игры (SteamVR/Oculus/WMR/Vive).
